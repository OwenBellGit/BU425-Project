...
getMetrics(getPreds(finalLR, "lr", val_data, 0.5), val_data$ChurnLabel)

#Random Forest
print("Random Forest")
getMetrics(getPreds(final_rf, "rf", val_data, 0.5), val_data$ChurnLabel)
```
...
library(pROC)
predictions_rf <- predict(final_rf, val_data, type = "prob")
binary_target <- ifelse(val_data$ChurnLabel == "Yes", 1, 0)
roc_values_rf <- roc(val_data$ChurnLabel, predictions_rf[, 2])
plot(roc_values_rf, main = "ROC Curve for Random Forest Model")
lines(roc_values_rf, col = "blue")

# Calculate AUC of random forest
auc_result_rf <- auc(roc_values_rf)
print(paste("AUC:", auc_result_rf))
```
...
predictions_lr <- predict(finalLR, val_data, type="resp")
binary_target <- ifelse(val_data$ChurnLabel == "Yes", 1, 0)
roc_values_lr <- roc(val_data$ChurnLabel, predictions_lr)
plot(roc_values_lr, main = "ROC Curve for Logistic Regression Model")
lines(roc_values_lr, col = "red")

# Calculate AUC of logistic regression
auc_result_lr <- auc(roc_values_lr)
print(paste("AUC:", auc_result_lr))
```



```{r}
# Combination of ROC curves for both models
plot(roc_values_lr, col = "red", main = "ROC Curve Comparison")
plot(roc_values_rf, col = "blue", add = TRUE)
legend("bottomright", legend = c("Logistic Regression", "Random Forest"), col = c("red", "blue"), lty = 1)
```
...
#Logistic Regression
print(paste0("Testing Error for final logistic regression is ", getError(getPreds(finalLR, "lr", test_data, 0.5), test_data$ChurnLabel)))
print(paste0("Validation Error for final logistic regression is ", getError(getPreds(finalLR, "lr", val_data, 0.5), val_data$ChurnLabel)))

#Random Forest
print(paste0("Testing Error for final random forest regression is ", ts.err.rf))
pred_rf_val <- predict(final_rf, newdata = val_data)
vl.err.rf = mean(val_data$ChurnLabel != pred_rf_val) 
print(paste0("Validation Error for final random forest regression is ", vl.err.rf))
```
