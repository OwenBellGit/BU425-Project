...
```{r}
oversampled_data <- ovun.sample(ChurnLabel ~ ., data = newdata, method = "over", N = nrow(newdata), seed = 123)$data #oversampled_data
set.seed(0)
```
...
sample <- sample(c(TRUE, FALSE), nrow(oversampled_data), replace=TRUE, prob=c(0.8,0.2))
...
m1 <- glm(ChurnLabel ~. ,data = train_data, family = "binomial")
pred_train <- predict(m1,train_data, type = "response")
pred <- predict(m1,test_data, type = "response")

#probabilities to values
pred_train<-ifelse(pred >0.5,"Yes","No")
pred_train<-as.factor(pred_train)

pred<-ifelse(pred >0.5,"Yes","No")
pred<-as.factor(pred)

tr.err.lr = mean(train_data$ChurnLabel != pred_train)
sprintf("Traing Error: %f",tr.err.lr)
```

```{r}
#Eval of the model
table(test_data$ChurnLabel, pred)
precision <- table(test_data$ChurnLabel, pred)[1]/(table(test_data$ChurnLabel, pred)[1] + table(test_data$ChurnLabel, pred)[3])
f1 <- 2 * (precision * sensitivity(test_data$ChurnLabel, pred)) / (precision + sensitivity(test_data$ChurnLabel, pred))

sprintf("Accuracy Score: %f",mean(pred == test_data$ChurnLabel))
sprintf("Specificity Score: %f",specificity(test_data$ChurnLabel, pred))
sprintf("Recall/Sensitivity Score: %f",sensitivity(test_data$ChurnLabel, pred))
sprintf("Precision Score: %f",precision)
sprintf("F1 Score: %f",f1)
sprintf("Test Error: %f",mean(test_data$ChurnLabel != pred))
```
Using Logistic Regression we are able to predict customer churn at an 82% accuracy and an F1 score of about 88%.
...
#library(glmnet)
x<-train_data%>%select(-ChurnLabel)
y<- as.numeric(train_data$ChurnLabel)

#Lasso
lasso <- glmnet(as.matrix(x),as.numeric(train_data$ChurnLabel), alpha = 1, family = "binomial")
pred_lasso <- predict(lasso,as.matrix(test_data%>%select(-ChurnLabel)), type = "response")
pred_lasso_train <- predict(lasso,as.matrix(train_data%>%select(-ChurnLabel)), type = "response")
pred_lasso<-ifelse(pred_lasso >0.5,"Yes","No")
pred_lasso_train<-ifelse(pred_lasso_train >0.5,"Yes","No")

sprintf("Lasso Traing Error: %f",mean(train_data$ChurnLabel != pred_lasso_train))
...
#Ridge
ridge <- glmnet(as.matrix(x),as.numeric(train_data$ChurnLabel), alpha = 0, family = "binomial")
pred_ridge <- predict(ridge,as.matrix(test_data%>%select(-ChurnLabel)), type = "response")
pred_ridge_train <- predict(ridge,as.matrix(train_data%>%select(-ChurnLabel)), type = "response")
pred_ridge<-ifelse(pred_ridge >0.5,"Yes","No")
pred_ridge_train<-ifelse(pred_ridge_train>0.5,"Yes","No")

sprintf("Ridge Traing Error: %f",mean(train_data$ChurnLabel != pred_ridge_train))
...
tgrf <- data.frame(mtry=7)
set.seed(0)
final_rf <- train(ChurnLabel ~ ., data = train_data,
                  method = "rf",
...
```{r}
# look at the test errors and hyper tune
pred_rf_train <-predict(final_rf, newdata = train_data)
yhat.train.rf = (predict(final_rf,train_data))
tr.err.rf = mean(train_data$ChurnLabel != pred_rf_train) 
tr.err.rf
